...
stopuser = {}
token = '6958307230:AAF_UsQA2KEKjx6tWM7vXW81PhIV4-ETjTg'
	...
	def my_function():
	  ...
	  "timer": "none",
		...
		if BL == '𝗙𝗥𝗘𝗘':	
			...
			contact_button = types.InlineKeyboardButton(text="✨ 𝗢𝗪𝗡𝗘𝗥  ✨", url="https://t.me/FF_B_M")
			keyboard.add(contact_button)
			random_number = random.randint(33, 82)
			photo_url = f'https://t.me/https:/liTEjtY_xg80OGM0/{random_number}'
...
𝒀𝑶𝑼𝑹 𝑷𝑳𝑨𝑵 𝑵𝑶𝑾 {BL}</b>
	''',reply_markup=keyboard)
			return
		keyboard = types.InlineKeyboardMarkup()
		contact_button = types.InlineKeyboardButton(text="✨ 𝗝𝗢𝗜𝗡 ✨", url="https://t.me/mosko733")
		keyboard.add(contact_button)
		username = message.from_user.first_name
		random_number = random.randint(33, 82)
		photo_url = f'https://t.me/liTEjtY_xg80OGM0/{random_number}'
...
𝘽𝙤𝙩 𝘽𝙮 @FF_B_M''', reply_markup=mes)
...
def start(message):
	def my_function():
		...
		try:
			...
			pas ='NARUTO-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))
			current_time = datetime.now()
